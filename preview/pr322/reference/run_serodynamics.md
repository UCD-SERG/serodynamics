...
- shape = shape parameter, estimated for power decay and fixed at 1 for
  exponential decay
...
- file_mod:
  ...
  The model file path. If `NULL`, a built-in model file is selected
  based on `decay_type`. If supplied, its parameterization must be
  ...
  - `population_params`: Optionally included modeled population
    ...
    - `Population_Parameter` identifies which modeled population
      ...
      - `mu.par` = The population means of the host-specific model
        ...
        transformed; shape is not included for exponential decay.
...
#> Welcome to JAGS 4.3.2 on Thu Sep 10 09:23:10 2026
...
#> Following the progress of chain 1 (the program will wait for all chains
#> to finish before continuing):
#> Welcome to JAGS 4.3.2 on Thu Sep 10 09:24:12 2026
#> JAGS is free software and comes with ABSOLUTELY NO WARRANTY
#> Loading module: basemod: ok
#> Loading module: bugs: ok
#> . . Reading data file data.txt
