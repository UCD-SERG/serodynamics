...
test_that("JAGS model gives consistent results with the same RNG inputs", {
  ...
  # Remove non-relevant components for comparison
  jags.post2$timetaken <- NULL
  jags.post$timetaken <- NULL
  jags.post2$runjags.version <- NULL
  jags.post$runjags.version <- NULL
  
  # Test if the remaining results are the same
  expect_equal(jags.post2, jags.post)
})

# To run this test:
# devtools::test() or testthat::test_file("tests/testthat/test-model.R")