...
suppressWarnings({
  ...
  test_that("JAGS model gives consistent results with the same RNG inputs", {
    ...
    # Compare results while excluding non-relevant components
    expect_equal(jags.post2[-which(names(jags.post2) %in% c("timetaken", "runjags.version"))],
                 jags.post[-which(names(jags.post) %in% c("timetaken", "runjags.version"))])
  })
})
