...
```{r, echo=FALSE, message=FALSE}
# Here we assume that the first index for the third dimension (antigens) is "IgG"
observed_igg <- longdata$logy[,,1]

# Flatten the observed data into a vector for comparison
observed_igg_flat <- as.vector(observed_igg)

# Step 1: Clean the observed data by removing NAs
observed_igg_flat_clean <- observed_igg_flat[!is.na(observed_igg_flat)]
observed_iga_flat_clean <- observed_iga_flat[!is.na(observed_iga_flat)]

# Step 2: Prepare and clean the posterior samples (yrep)
# Flatten yrep and remove corresponding NA indices to match the cleaned observed data
# Here, flattening the first chain from the MCMC list
yrep_igg <- as.matrix(mcmc_list[[1]])  # Assume yrep is structured appropriately

# Optionally, if yrep needs to be filtered to match the cleaned observed data:
yrep_igg_clean <- yrep_igg[!is.na(observed_igg_flat), ]

# Step 3: Perform posterior predictive checks using bayesplot

# Check for IgG
ppc_dens_overlay(
  y = observed_igg_flat_clean, 
  yrep = yrep_igg_clean
) + 
  ggtitle("Posterior Predictive Check: IgG")

# If you also want to check for IgA:
# Assuming you've prepared and cleaned yrep_iga similarly to yrep_igg
yrep_iga <- as.matrix(mcmc_list[[2]])  # Example if second chain corresponds to IgA
yrep_iga_clean <- yrep_iga[!is.na(observed_iga_flat), ]

ppc_dens_overlay(
  y = observed_iga_flat_clean, 
  yrep = yrep_iga_clean
) + 
  ggtitle("Posterior Predictive Check: IgA")
