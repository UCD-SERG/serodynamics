...
process_antibody_predictions <- function(dat, param_medians_wide, file_mod, strat = "bldculres") {
  ...
  # Step 3: Update dataset and join subject mapping
  dat_update <- dat %>%
    mutate(antigen_iso = case_when(
      antigen_iso == 1 ~ "HlyE_IgA",
      antigen_iso == 2 ~ "HlyE_IgG",
  ...
  # Step 10: Process JAGS output for re-run model
  param_medians_wide_2 <- process_jags_output(jags_post2, remove_last_subject = TRUE)
