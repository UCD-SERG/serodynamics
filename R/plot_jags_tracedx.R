...
#' entered in quotes.
#' @return A [list] of [ggplot] objects producing trace plots for all the specified
#' input.
#' @export
#' @examples
    ...
    for (j in iso) {
      ...
      visualize_jags_plot <- visualize_jags_plot |>
        ...
        dplyr::mutate(Parameter = paste0("iso = ", j, ", parameter = ",
                                         ...
                                         i)) |>
        dplyr::mutate(value = ifelse(Parameter_sub == "y1" | Parameter_sub == "alpha", 
                             log(value), value))
      # Assigning attributes, which are needed to run ggs_trace
