#' @title Simulate longitudinal data (user-supplied trajectory)
#' @author Kwan Ho Lee
#' @description
#' `simulate_multi_b_long2()` simulates subject-level latent parameters with a
...
#' trajectory is computed by a **vectorized** function we supply.
#'
#' @inheritParams simulate_multi_b_long
#' @param ab_vec A vectorized trajectory with signature
#'   `f(t, y0, y1, t1, alpha, shape)`. If `NULL`, a default
#'   vectorized wrapper of `serodynamics::ab()` is used.
#'
#' @return A list with:
#' \itemize{
...
#' @export
#' @example inst/examples/examples-simulate_multi_b_long2.R
simulate_multi_b_long2 <- function(
  ...
  # default trajectory (set inside to avoid lints about unknown symbols)
  if (is.null(ab_vec)) {
    ab_vec <- serodynamics:::make_ab_vec()
  }
  ...
  dat <- purrr::map_dfr(seq_len(n_id), function(i) {
    ...
    tibble::tibble(
      ...
      # avoid tidy-select: use data masking with .data$
      dplyr::transmute(
