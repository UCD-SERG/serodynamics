...
plot_jags_dens <- function(data=jags.post,iso=unique(visualize_jags_sub$Iso_type), param=unique(visualize_jags_sub$Parameter_sub)) {
  ...
  densplot <- ggs_density(visualize_jags_plot)
  densplot
}

#Example call
# plot_jags_dens(jags.post,"hlya_IgA","y1")
# plot_jags_dens(jags.post,c("hlya_IgG","hlya_IgA"),"y1")
