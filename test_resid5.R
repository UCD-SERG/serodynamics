...
# Extract specific observations where visit_num is 5
...
process_jags_output <- function(jags_post, remove_last_subject = TRUE) {
  ...
  if (remove_last_subject) {
    max_subject <- max(jags_processed$Subject, na.rm = TRUE)  # Identify the highest subject number
  ...
  # Step 5: Reshape into wide format (one row per subject-antigen pair, with parameters as columns)
...
process_antibody_predictions <- function(dat, param_medians_wide) {
  # Step 1: Create mapping for id
  unique_ids <- unique(dat$id)  # Get unique subject IDs
  subject_mapping <- data.frame(
    id = unique_ids,
    Subject = seq_along(unique_ids)  # Assign sequential numbers
  )
  
  # Step 2: Convert antigen_iso to character for consistency
  param_medians_wide <- param_medians_wide %>%
    mutate(antigen_iso = case_when(
      antigen_iso == 1 ~ "HlyE_IgA",
      antigen_iso == 2 ~ "HlyE_IgG",
      TRUE ~ as.character(antigen_iso)  # Ensure all values are characters
    ))
  
  # Step 3: Update the original dataset to match subjects and antigen_iso
  dat_update <- dat %>%
    mutate(antigen_iso = case_when(
      antigen_iso == 1 ~ "HlyE_IgA",
      antigen_iso == 2 ~ "HlyE_IgG",
    ...
    left_join(subject_mapping, by = "id")  # Assign subject numbers
  
  # Step 4: Define the antibody decay function
  ab <- function(t, y0, y1, t1, alpha, shape) {
    beta <- log(y1 / y0) / t1
    yt <- ifelse(t <= t1,
                 y0 * exp(beta * t), 
  ...
  # Step 5: Join with modeled parameters and compute predicted results
  dat_update <- dat_update %>%
    left_join(param_medians_wide, by = c("Subject", "antigen_iso")) %>%
    rowwise() %>%
...
dat_updated <- process_antibody_predictions(dat, param_medians_wide_128)
...
compute_residuals_and_prepare_data <- function(dat_update) {
  # Step 1: Compute residuals and reorganize data
  ...
  # Step 2: Prepare data for run_mod by keeping only abs_residual as result
  dat_resid_modified <- dat_resid %>%
    ...
    select(Country, id, sample_id, bldculres, antigen_iso, studyvisit, 
           dayssincefeveronset, result, visit_num)  # Reorder columns
  
  return(dat_resid_modified)
}


dat_resid_modified <- compute_residuals_and_prepare_data(dat_update)
...
param_medians_wide_128_2 <- process_jags_output(nepal_sees_jags_post2, remove_last_subject = TRUE)

# View the result
head(param_medians_wide_128_2)

################################################################################
## Using these 5 median parameters to plot the residual curve
