...
Here $\mu_x(\cdot)$ is your antibody kinetics function (rise/decay, etc.). Noise $\sigma_j$ is assay/measurement variability.
...
Key points: - **Independence given the population:** once you condition on $(\mu_j,\Sigma_{p,j})$, a brand-new person is independent of the existing data. $$
...
When you show the *population* summary, you are typically visualizing $\mu_j$ (and maybe an uncertainty band for $\mu_j$ itself). This reflects uncertainty in the mean but **does not** add the between-person scatter $\Sigma_{p,j}$.
...
-   **No person-to-person wiggle:** you don’t add $\Sigma_{p,j}$ here

**Rule of thumb:** - Use *population* when you want the **typical (mean) kinetics**.\
- Use *new person* when you want what you’d expect for a **random individual**.

------------------------------------------------------------------------

### 4) Why the posterior for $(\mu_j, \Sigma_{p,j})$ isn’t “just the average of individual fits”

A common misconception: if you fit each person separately and then average, you do **not** recover the hierarchical posterior.
