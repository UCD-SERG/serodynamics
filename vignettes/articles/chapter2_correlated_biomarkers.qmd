...
-   Current implementation: block-diagonal covariance (independent biomarkers)\
-   Extension target: full $\Sigma_B$ to capture correlation between biomarkers
...
$$
\log(y_{\text{obs},ijk}) \sim \mathcal{N}(\eta_{ijk}, \tau_j^{-1})
$$ {#eq-1}

Where:

-   $y_{\text{obs},ijk}$: Observed antibody level for subject $i$, biomarker $j$, at visit/time index $k$
-   $\eta_{ijk}$: **expected log antibody level** at time $t_{ijk}$ (without measurement error), computed from the two-phase model using subject-level parameters $\theta_{ij}$
...
| Quantity | [@teunis2016] notation | serodynamics notation |
|------------------------|------------------------|------------------------|
| Pathogen growth rate | $\mu_0$ | $\gamma_b$ |
| Antibody growth rate | $\mu$ | $\gamma_y$ |
| Clearance parameter | $c$ | $\kappa$ |
| Decay-shape parameter | $r$ | $\rho$ |

: Notation mapping between Teunis (2016) and serodynamics. {#tbl-model-comparison}

------------------------------------------------------------------------

## Full Serokinetic Parameter Set (7 Parameters)

The underlying serokinetic system uses seven subject-level quantities:

$$
\left(y_{0,ij},\ b_{0,ij},\ \gamma_{b,ij},\ \gamma_{y,ij},\ \kappa_{ij},\ \alpha_{ij},\ \rho_{ij}\right)
$$

These define the full within-host dynamics for antibody and pathogen trajectories.
For the hierarchical model below, the Gaussian assumption is placed on the
**transformed 5-parameter serodynamic vector**, not directly on this 7-parameter set.

------------------------------------------------------------------------

## From 7 Serokinetic Parameters to 5 Serodynamic Parameters

-   For cross-sectional seroincidence work we focus on the seroresponse function
    $y(t)$; this reduces attention to a **5-parameter projection**:
