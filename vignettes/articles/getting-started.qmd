...
`run_mod()`, `plot_jags_trace()`, `plot_jags_dens()` and `plot_jags_Rhat()`
where this article calls `run_serodynamics()`, `plot_trace()`, `plot_density()`
and `plot_rhat()`, and it has no equivalent of `plot_ess()`. The code below
therefore requires the development version.
