...
#balance data accross ages and sites

# Create age bins
df <- d0 %>%
  filter(!is.na(age)) %>%
  mutate(age_bin = cut(age, breaks = seq(0, 60, by = 1), right = FALSE))

# Count available samples per country and bin
bin_counts <- df %>%
  count(country, age_bin)

# Find minimum counts per bin across countries
min_counts <- bin_counts %>%
  group_by(age_bin) %>%
  summarise(n_min = min(n))

# Join back to original data
df_counts <- df %>%
  left_join(min_counts, by = "age_bin")

# Sample per country and age bin
set.seed(123)


df_balanced <- df_counts %>%
  group_by(country, age_bin) %>%
  group_modify(~ {
    n_take <- unique(.x$n_min)
    if (is.na(n_take) || n_take == 0) {
      tibble() # skip if no samples in this bin
    } else {
...
ggplot(df_balanced, aes(x = age, fill = country)) + 
